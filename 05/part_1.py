...
seeds = list(map(lambda number: int(number), re.search(r"(?<=seeds:\s)(\d+ ?)+", data).group().split(" ")))
...
for i, res in enumerate(keys):
  ...
  maps[res.group(1)] = list(map(lambda item: list(map(lambda number: int(number), item.split(" "))), re.search(r"((\d+ ?)+\n)+(\d+ ?)+", data[(res.end() + 1):end]).group().split("\n")))
